...
public interface UserDailyExpMapper {
    @Insert("insert into user_daily_exp (user_info_id, exp_date, exp_got) values (#{userInfoId}, CURDATE(), COALESCE(#{expGot}, 0))")
    @Options(useGeneratedKeys = true, keyProperty = "id", keyColumn = "id")
    int insert(UserDailyExp dailyExp);

    @Select("select * from user_daily_exp where user_info_id = #{userId} and exp_date = CURDATE()")
    ...
    @Update("update user_daily_exp set exp_got = #{expGot} where user_info_id = #{userId} and exp_date = CURDATE()")
